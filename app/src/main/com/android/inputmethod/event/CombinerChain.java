    ...
    public void applyProcessedEvent(final Event event) {
        if (null != event) {
            ...
            if (Constants.CODE_DELETE == event.mKeyCode) {
                Timber.d("ducNQ : applyProcessedEvent: 1");
             //   mCombinedText.setLength(0);
                final int length = mCombinedText.length();
              //  Timber.d("ducNQapplyProcessedEvent : "+mCombinedText.toString());
                if (length > 0) {
               ...
               // Timber.d("ducNQapplyProcessedEvent : "+mCombinedText.length());
            } else {
            //    Timber.d("ducNQapplyProcessedEvent event : "+event.getTextToCommit().length());
            //    event.setTextToCommit("");
                Timber.d("ducNQ : applyProcessedEventss: "+event.getTextToCommit());
