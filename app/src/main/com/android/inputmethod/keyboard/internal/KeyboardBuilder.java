...
public class KeyboardBuilder<KP extends KeyboardParams> {
    ...
    public KeyboardBuilder(final Context context, @Nonnull final KP params) {
        ...
        mParams = params;
        Log.d("duongcv", "KeyboardBuilder: "+ mParams.mOccupiedHeight);
    ...
    private void parseKeyboardAttributes(final XmlPullParser parser) {
        ...
        try {
            ...
            int height = params.mId.mHeight;
            int heightEmoji = params.mId.mHeight;
            final int width = params.mId.mWidth;
            if(App.getInstance().mPrefs != null && App.getInstance().getTypeEditing() == com.tapbi.spark.yokey.util.Constant.TYPE_EDIT_NONE) {
                if (!App.getInstance().isScreenLandscape) {
                    int heightKbNew = (int) App.getInstance().mPrefs.getFloat(Constant.HEIGHT_KEYBOARD_NEW, 0);
                    height = heightKbNew != 0 ? heightKbNew : height;
                } else {
                    int heightKbNew = (int) App.getInstance().mPrefs.getFloat(Constant.HEIGHT_KEYBOARD_NEW_VERTICAL, 0);
                    height = heightKbNew != 0 ? heightKbNew : height;
//                    heightEmoji = height;
                }
            }
            params.mOccupiedHeight = height;
//            params.mOccupiedHeight = height > DisplayUtils.getScreenHeight()/2 ?  height*2/5 : height;
            params.mOccupiedWidth = width;
            params.mTopPadding = (int) keyboardAttr.getFraction(
                    R.styleable.Keyboard_keyboardTopPadding, params.mOccupiedHeight, params.mOccupiedHeight, 0);
            params.mBottomPadding = (int) keyboardAttr.getFraction(
                    R.styleable.Keyboard_keyboardBottomPadding, params.mOccupiedHeight, params.mOccupiedHeight, 0);
            ...
            params.mVerticalGap = (int) keyboardAttr.getFraction(
                    R.styleable.Keyboard_verticalGap, params.mOccupiedHeight, params.mOccupiedHeight, 0);
            ...
            final int baseHeightEmoji = heightEmoji - params.mTopPadding
                    - params.mBottomPadding + params.mVerticalGap;
            params.mBaseHeightEmoji = baseHeightEmoji;
            params.mDefaultRowHeightEmoji = (int) ResourceUtils.getDimensionOrFraction(keyboardAttr,
                    R.styleable.Keyboard_rowHeight, baseHeightEmoji, baseHeightEmoji / DEFAULT_KEYBOARD_ROWS);
...
//                final int height = row.getRowHeight();
                final int height = row.getRowHeightEmoji();
    ...
    private void endKeyboard() {
        ...
        Log.d("duongcv", "endKeyboard: "+ mParams.mOccupiedHeight);
    }
