...
public class KeyPreviewView extends TextView {
    ...
    private int TEXT_SIZE = CommonUtil.dpToPx(App.getInstance(), 10);
    ...
    private void setTextAndScaleX(final String text) {
        setTextScaleX(1.0f);
        setText(text);
        String textPopup = text;
        mPrefs = PreferenceManager.getDefaultSharedPreferences(getContext());
        String keyFont = mPrefs.getString(Constant.USING_FONT, Constant.FONT_NORMAL);
        CharSequence[] sp = font.getFont(keyFont);
        textPopup = CommonUtil.replaceTextFontOUTPUT(Constant.LIST_LANGUAGE_KEYBOARD_OTHER_QWERTY.contains(Settings.getLanguageKeyBoardCurrent()), sp, textPopup, text, font, keyFont);
        ...
        final int maxWidth = background.getIntrinsicWidth() - mBackgroundPadding.left - mBackgroundPadding.right;
