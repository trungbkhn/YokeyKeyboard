...
public final class KeyboardState {
    ...
    //todo rgb keyboard edit log debug
    private static final boolean DEBUG_EVENT = true;  //false;
    private static final boolean DEBUG_INTERNAL_ACTION = true; //false;
    ...
    public AlphabetShiftState mAlphabetShiftState = new AlphabetShiftState();
    ...
    public KeyboardState(final SwitchActions switchActions, KeyboardSwitcher kbsw) {
        keyboardSwitcher = kbsw;
    ...
    public void onCodeInput(final int code, final int currentAutoCapsState,
        ...
        Log.d("duongcv", "onCodeInput: 12345");
        // If the code is a letter, update keyboard shift state.
        if (Constants.isLetterCode(code)) {
            isAlphaFromEmoji = false;
            updateAlphabetShiftState(currentAutoCapsState, currentRecapitalizeState);
        } else if (code == Constants.CODE_ALPHA_FROM_EMOJI) {
            isAlphaFromEmoji = true;
            setAlphabetKeyboard(currentAutoCapsState, currentRecapitalizeState);
        } else if (code == Constants.CODE_ALPHA_FROM_EMOJI_SEARCH) {
            isAlphaFromEmoji = false;
            setAlphabetKeyboardSearchEmoji(currentAutoCapsState, currentRecapitalizeState);
        } else if (code == Constants.CODE_ALPHA_FROM_TRANSLATE) {
            isAlphaFromEmoji = false;
            setAlphabetKeyboardTranslate(currentAutoCapsState, currentRecapitalizeState);
        } else {
            isAlphaFromEmoji = false;
        }
    ...
    private void onRestoreKeyboardState(final int autoCapsFlags, final int recapitalizeMode) {
        ...
        Timber.d("ducNQ : onRestoreKeyboardStated: "+state.mIsEmojiMode);
        if (state.mIsEmojiMode) {
            setAlphabetKeyboard(autoCapsFlags, recapitalizeMode);// fix bug 471
            //setEmojiKeyboard();
            return;
        }
        // Symbol mode
    ...
    private void resetKeyboardStateToAlphabet(final int autoCapsFlags, final int recapitalizeMode) {
        ...
        if (mIsAlphabetMode) return;
        Timber.d("ducNQ : resetKeyboardStateToAlphabetd: ");
        mPrevSymbolsKeyboardWasShifted = mIsSymbolShifted;
        setAlphabetKeyboard(autoCapsFlags, recapitalizeMode);
        if (mPrevMainKeyboardWasShiftLocked) {
            setShiftLocked(true);
    ...
    private void setAlphabetKeyboardSearchEmoji(final int currentAutoCapsState,
                                                final int currentRecapitalizeState) {
        if (DEBUG_INTERNAL_ACTION) {
            Log.d(TAG, "setAlphabetKeyboard");
        }

        mSwitchActions.setAlphabetKeyboardSearchGif();
    ...
    private void setAlphabetKeyboard(final int autoCapsFlags, final int recapitalizeMode) {
        ...
        // todo: Duongcv update shift when back from emoji
//        int state = App.getInstance().mPrefs.getInt(Settings.SAVE_STATE_SHIFT_WHEN_CHANGE_SCREEN_EMOJI_STICKER_GIF, UNSHIFT);
//        if (state != UNSHIFT) {
//            setShifted(state);
//            mAlphabetShiftState.mState = state;
//            if (keyboardSwitcher.getMainKeyboardView().getThemeModel().getTypeKeyboard() != Constants.ID_CATEGORY_RGB) {
//                keyboardSwitcher.getMainKeyboardView().invalidateAllKeys();
//            }
//        }
    }
    ...
    public void setEmojiKeyboard() {
        ...
        Timber.d("ducNQ : setEmojiKeyboardaaa: ");
        mIsAlphabetMode = false;
        mIsEmojiMode = true;
        mRecapitalizeMode = RecapitalizeStatus.NOT_A_RECAPITALIZE_MODE;
        ...
        mPrevMainKeyboardWasShiftLocked = mAlphabetShiftState.isShiftLocked();
        mAlphabetShiftState.setShiftLocked(false);
        mSwitchActions.setEmojiKeyboard(true);
    }
    ...
    public void onResetKeyboardStateToAlphabet(final int autoCapsFlags,
        ...
        Timber.d("ducNQ : onResetKeyboardStateToAlphabetd: ");
    ...
    public void updateAlphabetShiftState(final int autoCapsFlags, final int recapitalizeMode) {
        if (!mIsAlphabetMode) return;
...
//                    setShifted(state);
//                    mAlphabetShiftState.mState = state;
//                    if (state == MANUAL_SHIFT)
//                        App.getInstance().mPrefs.edit().putInt(Settings.SAVE_STATE_SHIFT_WHEN_CHANGE_SCREEN_EMOJI_STICKER_GIF, UNSHIFT).apply();
//                }
            }
        }
    }
    ...
    public void onEvent(final Event event, final int autoCapsFlags, final int recapitalizeMode) {
        ...
        if (Constants.isLetterCode(code)) {
            ...
            isAlphaFromEmoji = false;
//            App.getInstance().mPrefs.edit().putInt(Settings.SAVE_STATE_SHIFT_WHEN_CHANGE_SCREEN_EMOJI_STICKER_GIF, mAlphabetShiftState.mState).apply();
            Timber.d("duongcv emoji");
            EventBus.getDefault().post(new MessageEvent(Constant.EVENT_EMOJI));
//            setEmojiKeyboard();
        } else if (code == Constants.CODE_ALPHA_FROM_EMOJI) {
            isAlphaFromEmoji = true;
            setAlphabetKeyboard(autoCapsFlags, recapitalizeMode);
            ...
            isAlphaFromEmoji = false;
            setAlphabetKeyboardSearchEmoji(autoCapsFlags, recapitalizeMode);
        } else if (code == Constants.CODE_ALPHA_FROM_TRANSLATE) {
            isAlphaFromEmoji = false;
            setAlphabetKeyboardTranslate(autoCapsFlags, recapitalizeMode);
        } else {
            isAlphaFromEmoji = false;
        }
    }
    ...
    private void setAlphabetKeyboardTranslate(final int currentAutoCapsState,
                                              final int currentRecapitalizeState) {
        mSwitchActions.setAlphabetKeyboardTranslate();
        mIsAlphabetMode = true;
        mIsEmojiMode = false;
        mIsSymbolShifted = false;
        mRecapitalizeMode = RecapitalizeStatus.NOT_A_RECAPITALIZE_MODE;
