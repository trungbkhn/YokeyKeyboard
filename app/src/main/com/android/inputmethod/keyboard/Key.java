...
public class Key implements Comparable<Key> {
    ...
    public Key(@Nullable final String label, final int iconId, final int code,
        ...
        mIconId = iconId;
        if (mCode == Constants.CODE_SHIFT_ENTER || mCode == Constants.CODE_ENTER){
            Timber.d("duongcv getIcon" + iconId);
        }
    ...
    public void setHeightKey(int heightKey) {
        mHeight = heightKey;
    }
    ...
    public Key(@Nullable final String keySpec, @Nonnull final TypedArray keyAttr,
        ...
        if (mCode == Constants.CODE_SHIFT_ENTER || mCode == Constants.CODE_ENTER){
            Timber.d("duongcv getIcon" + mIconId);
        }
        ...
        if (code == Constants.CODE_ENTER || code == Constants.CODE_SHIFT_ENTER){
            Timber.d("duongcv");
        }
    ...
    public void setCode(int mCoded) {
        mCode = mCoded;
    }

    public boolean checkCode() {
        return mCode == Constants.CODE_ENTER;
    }
    ...
    public final int selectTextSize(final KeyDrawParams params) {
        switch (mLabelFlags & LABEL_FLAGS_FOLLOW_KEY_TEXT_RATIO_MASK) {
            ...
            case LABEL_FLAGS_FOLLOW_KEY_LARGE_LETTER_RATIO:
                return (int) (params.mLargeLetterSize * 0.7);
    ...
    public final int selectHintTextSize(final KeyDrawParams params) {
        if (hasHintLabel()) {
            return (int) (params.mHintLabelSize * 0.9);
        }
...
//        if (mCode == Constants.CODE_ENTER){
//            Timber.d("duongcv getIcon" + iconId);
//        }else if (mCode == Constants.CODE_SHIFT_ENTER){
//            Timber.d("duongcv getIcon" + iconId);
//        }
        if (icon != null) {
            icon.setAlpha(alpha);
