...
public final class PointerTracker implements PointerTrackerQueue.Element,
        ...
        if (key.isEnabled() || altersCode) {
            ...
            if (code == Constants.CODE_OUTPUT_TEXT) {
                ...
                if (mKeyboard.hasProximityCharsCorrection(code)) {
                    Log.d("duongcv", "callListenerOnCodeInput: ");
                    sListener.onCodeInput(code, x, y, isKeyRepeat,false);
                } else {
                    sListener.onCodeInput(code,
                            Constants.NOT_A_COORDINATE, Constants.NOT_A_COORDINATE, isKeyRepeat,false);
                }
            }
        }
    }
    ...
    private void onUpEventInternal(final int x, final int y, final long eventTime) {
        ...
        Log.d("duongcv", "onUpEventInternal: ");
        if (isShowingMoreKeysPanel()) {
            if (!mIsTrackingForActionDisabled) {
                final int translatedX = mMoreKeysPanel.translateX(x);
                final int translatedY = mMoreKeysPanel.translateY(y);
            ...
            Log.d("duongcv", "onUpEventInternal: isShowingMoreKeysPanel");
            return;
        }
        ...
        if (sInGesture) {
            ...
            showGestureTrail();
            Log.d("duongcv", "onUpEventInternal: sInGesture");
            return;
        }

        if (mIsTrackingForActionDisabled) {
            Log.d("duongcv", "onUpEventInternal: mIsTrackingForActionDisabled");
            return;
        }
        if (currentKey != null && currentKey.isRepeatable()
                && (currentKey.getCode() == currentRepeatingKeyCode) && !isInDraggingFinger) {
            Log.d("duongcv", "onUpEventInternal: cuoi");
            return;
        }
    ...
    public void cancelTrackingForAction() {
        if (isShowingMoreKeysPanel()) {
            Log.d("duongcv", "cancelTrackingForAction: true");
            return;
        }
        Log.d("duongcv", "cancelTrackingForAction: ");
    ...
    public void onLongPressed() {
        ...
        if (key.hasNoPanelAutoMoreKey()) {
            ...
            sListener.onCodeInput(moreKeyCode, Constants.NOT_A_COORDINATE,
                    Constants.NOT_A_COORDINATE, false /* isKeyRepeat */,false);
    ...
    private void detectAndSendKey(final Key key, final int x, final int y, final long eventTime) {
        ...
        final int code = key.getCode();
        Log.d("duongcv", "detectAndSendKey: ");
