...
public final class WordComposer {
    ...
    private final void refreshTypedWordCache() {
        ...
        Log.d("duongcv", "refreshTypedWordCache: " + mTypedWordCache);
    ...
    public void setmCapitalizedMode(int mCapitalizedMode) {
        this.mCapitalizedMode = mCapitalizedMode;
    }

    public void setCharAt(int pos, String ch) {
        StringBuilder s = new StringBuilder(mTypedWordCache);
        s.replace(pos, pos + ch.length(), ch);
        mTypedWordCache = s;
        Log.d("duongcv", "setCharAt: " + mTypedWordCache);
        try {
            if (Character.isUpperCase(s.charAt(pos)) && !Character.isUpperCase(ch.charAt(0)))
                mCapsCount--;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void applyProcessedEvent(final Event event) {
        Timber.d("ducNQ : applyProcessedEvented: 1");
    ...
    public boolean moveCursorByAndReturnIfInsideComposingWord(final int expectedMoveAmount) {
        ...
        if (expectedMoveAmount >= 0) {
            ...
            while (actualMoveAmount > expectedMoveAmount && cursorPos > 0) {
                --cursorPos;
                if (codePoints.length > cursorPos) {
                    actualMoveAmount -= Character.charCount(codePoints[cursorPos]);
                }
            }
        }
        ...
        Timber.d("ducNQ : applyProcessedEvented: 2");
    ...
    public void setTypedWord(String word) {
         mTypedWordCache = word;
    }
    ...
    public LastComposedWord commitWord(final int type, final CharSequence committedWord,
        ...
        Timber.e("hachung mTypedWordCache.toString(): "+mTypedWordCache.toString()+"/committedWord: "+committedWord+"/separatorString: "+separatorString);
    ...
    void setTypedWordCacheForTests(String typedWordCacheForTests) {
        ...
        Timber.d("ducNQ : mTypedWordCache: 3");
    }
    ...
    private void addKeyInfo(final int codePoint, final Keyboard keyboard) {
        Key key = keyboard.getKey(codePoint);
        if (key != null) {
    ...
    public void add(int primaryCode, int keyX, int keyY) {
        final int newIndex = size();
        StringBuilder s = new StringBuilder(mTypedWordCache);
        s.append((char) primaryCode);
        mTypedWordCache = s;
        Log.d("duongcv", "add: "+ mTypedWordCache);
        refreshSize();
        if (newIndex < MAX_WORD_LENGTH) {
            mInputPointers.addPointerAt(newIndex, keyX, keyY, 0, 0);
        }
    ...
    public final void refreshSize() {
        Timber.d("ducNQ : refreshSized: "+mTypedWordCache);
        mCodePointSize = Character.codePointCount(mTypedWordCache, 0, mTypedWordCache.length());
        mCursorPositionWithinWord = mCodePointSize;
    }
}
