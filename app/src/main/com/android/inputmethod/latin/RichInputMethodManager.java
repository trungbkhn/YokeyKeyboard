...
public class RichInputMethodManager {
    ...
    private int currentSubTypeId = Integer.MIN_VALUE;
    ...
    public InputMethodSubtype rollbackToDefaultSubtype() {
        ArrayList<InputMethodSubtype> inputMethodSubtypesLocal = getInputSubtypeLocal();
        if (inputMethodSubtypesLocal != null && !inputMethodSubtypesLocal.isEmpty()) {
            String languageFull = null;
            String language = null;
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                LocaleList localeList = LocaleList.getDefault();
                if (localeList != null && !localeList.isEmpty() && localeList.get(0) != null) {
                    languageFull = localeList.get(0).toString();
                    language = localeList.get(0).getLanguage();
                }
            } else {
                languageFull = Locale.getDefault().toString();
                language = Locale.getDefault().getLanguage();
            }
            if (languageFull == null || languageFull.isEmpty()) {
                languageFull = Constant.LOCALE_LANGUAGE_DEFAULT;
            }
            if (language == null || language.isEmpty()) {
                language = Constant.LOCALE_DEFAULT;
            }
            List<LanguageEntity> list = App.getInstance().keyboardLanguageRepository.mLanguageEntities;
            String languageCurrent = App.getInstance().mPrefs.getString(Constant.LOCALE_CURRENT_LANGUAGE,Locale.getDefault().getLanguage());
            indexLanguage = 0;
            if(!list.isEmpty()) {
                for (int i = 0; i < list.size(); i++) {
                    if (list.get(i) != null && list.get(i).locale != null && list.get(i).locale.contains(languageCurrent)) {
                        indexLanguage = i;
                        break;
                    }
                }
            }
            if (indexLanguage < list.size()) {
                if(list.get(indexLanguage) != null){
                    languageFull = list.get(indexLanguage).locale;
                    language = languageFull;
                }

               // Timber.d("ducNQ : rollbackToDefaultSubtype: "+indexLanguage);
               // Log.d("duongcv", "rollbackToDefaultSubtype: 1: " + list.get(indexLanguage).locale);
            }
            for (InputMethodSubtype inputMethodSubtype : inputMethodSubtypesLocal) {
                if (inputMethodSubtype.getLocale().equals(languageFull) || inputMethodSubtype.getLocale().contains(language)) {
                    return inputMethodSubtype;
                }
            }
            return inputMethodSubtypesLocal.get(0);
        }
        InputMethodSubtype subtype = mImmWrapper.mImm.getCurrentInputMethodSubtype();
...
//        return switchToNextInputMethodAndSubtype(token);
        ...
        return switchToNextInputSubtypeInThisImeRGB(token, onlyCurrentIme);
    }

    private boolean switchToNextInputSubtypeInThisImeRGB(final IBinder token,
        ...
        if (App.getInstance().keyboardLanguageRepository == null) return false;
        List<LanguageEntity> list = App.getInstance().keyboardLanguageRepository.mLanguageEntities;
        ...
        int nextIndex = (currentIndex + 1) % enabledSubtypes.size();
        if (nextIndex == indexLanguage && list != null) {
        ...
        if (nextIndex > INDEX_NOT_FOUND) {
            indexLanguage = nextIndex;
            if (nextIndex < enabledSubtypes.size() && App.getInstance().mPrefs != null) {
                final InputMethodSubtype nextSubtype = enabledSubtypes.get(nextIndex);
                currentSubTypeId = nextSubtype.hashCode();
                setInputMethodAndSubtype(token, nextSubtype);
                App.getInstance().mPrefs.edit().putInt(Constant.LOCALE_CURRENT_SUBTYPE_ID, currentSubTypeId).apply();
                App.getInstance().mPrefs.edit().putString(Constant.LOCALE_CURRENT_LANGUAGE, enabledSubtypes.get(nextIndex).getLocale()).apply();
                return true;
            }
        }
        return false;
    ...
    public List<InputMethodSubtype> getEnabledInputSubtypesList() {
        return getEnabledInputSubtypesArrList();
    }

    private ArrayList<InputMethodSubtype> getEnabledInputSubtypesArrList() {
        try {
            return AdditionalSubtypeUtils.createAdditionalSubtypesArrayList(App.getInstance().keyboardLanguageRepository.mLanguageEntities);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public InputMethodSubtype getCurrentInputMethodSubtype() {
        List<InputMethodSubtype> inputMethodSubtypes = getEnabledInputSubtypesList();
        if (inputMethodSubtypes != null && inputMethodSubtypes.size() > 0) {
            String locale = App.getInstance().mPrefs.getString(Constant.LOCALE_CURRENT_LANGUAGE, "");
            if (locale != null && !locale.isEmpty()) {
                for (InputMethodSubtype inputMethodSubtype : inputMethodSubtypes) {
                    if (inputMethodSubtype.getLocale().equals(locale)) {
                        if (currentSubTypeId != Integer.MIN_VALUE) {
                            if (inputMethodSubtype.hashCode() == currentSubTypeId) {
                                return inputMethodSubtype;
                            }
                        } else {
                            currentSubTypeId = inputMethodSubtype.hashCode();
                            App.getInstance().mPrefs.edit().putInt(Constant.LOCALE_CURRENT_SUBTYPE_ID, currentSubTypeId).apply();
                            return inputMethodSubtype;
                        }
                        // return AdditionalSubtypeUtils.createAdditionalSubtype(inputMethodSubtype.prefSubtype);

                    }
                }
            }
//            int index = new Random().nextInt(inputMethodSubtypes.size());
//            Log.d("duongcv", "getCurrentInputMethodSubtype: "+ index +":"+inputMethodSubtypes.size());
            App.getInstance().mPrefs.edit().putString(Constant.LOCALE_CURRENT_LANGUAGE, inputMethodSubtypes.get(0).getLocale()).apply();
            // return AdditionalSubtypeUtils.createAdditionalSubtype(inputMethodSubtypes[0].prefSubtype);
            return inputMethodSubtypes.get(0);
        }
        // todo: get subtype default
        return getInputSubtypeLocalDefault();
    }

    public ArrayList<InputMethodSubtype> getInputSubtypeLocal() {
        ArrayList<InputMethodSubtype> inputMethodSubtypes = new ArrayList<>();
        InputMethodInfo inputMethodInfo = getInputMethodInfoOfThisIme();
        ...
        for (int i = 0; i < count; i++) {
            ...
            inputMethodSubtypes.add(subtype);
        }
        return inputMethodSubtypes;
    }

    public InputMethodSubtype getInputSubtypeLocalDefault() {
        ArrayList<InputMethodSubtype> inputMethodSubtypesLocal = getInputSubtypeLocal();
        if (inputMethodSubtypesLocal != null && !inputMethodSubtypesLocal.isEmpty()) {
            // todo: try get subtype by locale current
            String locale = App.getInstance().mPrefs.getString(Constant.LOCALE_CURRENT_LANGUAGE, "");
            if (locale != null && !locale.isEmpty()) {
                for (InputMethodSubtype inputMethodSubtype : inputMethodSubtypesLocal) {
                    if (inputMethodSubtype.getLocale().equals(locale)) {
            ...
            // todo: try get subtype by locale system language
            String languageFull = null;
            String language = null;
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                LocaleList localeList = LocaleList.getDefault();
            ...
            // todo: try get subtype index 0
            return inputMethodSubtypesLocal.get(0);
        }
        // todo: try get subtype current ime
        InputMethodSubtype subtype = mImmWrapper.mImm.getCurrentInputMethodSubtype();
        return subtype;
    }
...
//    }
    ...
    private static class InputMethodInfoCache {
        ...
        public synchronized InputMethodInfo getInputMethodOfThisIme() {
            ...
            List<InputMethodInfo> list = new InputMethodManagerCompatWrapper(App.getInstance()).mImm.getInputMethodList();
            for (final InputMethodInfo imi : list) {
                if (imi.getPackageName().equals(mImePackageName)) {
                    mCachedThisImeInfo = imi;
                    return imi;
                }
...
//                    imi, allowsImplicitlySelectedSubtypes);
            ...
            final List<InputMethodSubtype> result = RichInputMethodManager.getInstance().getEnabledInputSubtypesList();
    ...
    public void setLocale(Locale locale) {
        mCurrentRichInputMethodSubtype.setmLocale(locale);
    }
