...
public class VietnameseSpellChecker {
    ...
    // public static boolean isVietnameseWord(CharSequence charSequence) {
    // 	int length = charSequence.length() - 1;
    // 	while (length >= 0) {
    // 		char currentChar = charSequence.charAt(length);
    // 		int i = 0;
    // 		while (i < NON_VIETNAMESE_CHARACTERS.length) {
    // 			if ((currentChar == NON_VIETNAMESE_CHARACTERS[i][0] ||
    // 					currentChar == NON_VIETNAMESE_CHARACTERS[i][0] + 32) && length >= NON_VIETNAMESE_CHARACTERS[i][1]) {
    // 				return false;
    // 			}
    // 			i++;
    // 		}
