...
import static com.tapbi.spark.yokey.util.Constant.LIST_LANGUAGE_KEYBOARD_OTHER_QWERTY;
...
public final class InputLogic {
    ...
    public boolean isSpaceSuggestion=false;
    // Never null.
    ...
    public void startInput(final String combiningSpec, final SettingsValues settingsValues) {
        ...
        mDeleteCount = 0;
        Timber.e("hachung mSpaceState: NONE");
    ...
    public void setmDeleteCount(int mDeleteCount) {
    ...
    public InputTransaction onTextInput(final SettingsValues settingsValues, final Event event,
        ...
        if (SpaceState.PHANTOM == mSpaceState) {
            Timber.e("hachung insertAutomaticSpaceIfOptionsAndTextAllow:" + 1);
            insertAutomaticSpaceIfOptionsAndTextAllow(settingsValues);
        }
        Timber.e("hachung  : commit: " + text);
    ...
    public InputTransaction onPickSuggestionManually(final SettingsValues settingsValues,
        ...
        if (suggestion.length() == 1 && suggestedWords.isPunctuationSuggestions()) {
            ...
            Log.d("duongcv", "onPickSuggestionManually: ");
            return onCodeInput(settingsValues, event, keyboardShiftState,
                    currentKeyboardScriptId, handler, false);
        }
        ...
        if (SpaceState.PHANTOM == mSpaceState && suggestion.length() > 0
            ...
            if (!settingsValues.isWordSeparator(firstChar)
                ...
                Timber.e("hachung insertAutomaticSpaceIfOptionsAndTextAllow:" + 2);
                insertAutomaticSpaceIfOptionsAndTextAllow(settingsValues);
            }
        }
        ...
        // Space state must be updated before calling updateShiftState
        Timber.e("hachung PHANTOM:");
        mSpaceState = SpaceState.PHANTOM;
        isSpaceSuggestion=true;
        inputTransaction.requireShiftUpdate(InputTransaction.SHIFT_UPDATE_NOW);
    ...
    public InputTransaction onCodeInput(final SettingsValues settingsValues,
                                        ...
                                        final int currentKeyboardScriptId, final LatinIME.UIHandler handler, boolean isEmoji) {
        mWordBeingCorrectedByCursor = null;
        ...
        while (null != currentEvent) {
            if (currentEvent.isConsumed()) {
                ...
                handleNonFunctionalEvent(currentEvent, inputTransaction, handler, isEmoji);
            }
    ...
    public String replaceTextSuggestion(String suggestion) {//translate language by font
        StringBuilder suggestionsWordsResult = new StringBuilder();
        String key_font = Objects.requireNonNull(App.getInstance().mPrefs).getString(Constant.USING_FONT, Constant.FONT_NORMAL);
        CharSequence[] sp = Objects.requireNonNull(App.getInstance().fontRepository).font.getFont(key_font);
        for (int i = 0; i < suggestion.length(); i++) {
            String charAt = suggestion.charAt(i) + "";
            charAt = CommonUtil.replaceTextFontOUTPUT(LIST_LANGUAGE_KEYBOARD_OTHER_QWERTY.contains(Settings.getLanguageKeyBoardCurrent()), sp, charAt, charAt, App.getInstance().fontRepository.font, key_font);
            suggestionsWordsResult.append(charAt);
        }
        return suggestionsWordsResult.toString();
    }
    ...
    private String getWordAutoCorrect(String wordCurrent, SuggestedWords mSuggestedWords) {
        if (App.Companion.getInstance().mPrefs.getBoolean(Settings.PREF_AUTO_CORRECTION, false) && mSuggestedWords != null
                && mSuggestedWords.mSuggestedWordInfoList != null && mSuggestedWords.mSuggestedWordInfoList.size() > 0 &&
                wordCurrent != null && !wordCurrent.trim().isEmpty() && wordCurrent.length() > 1) {
            ArrayList<String> listWord = new ArrayList<>();
            for (SuggestedWords.SuggestedWordInfo wordInfo : mSuggestedWords.mSuggestedWordInfoList) {
                if (wordCurrent.equals(wordInfo.mWord)) {
                    return wordCurrent;
                }
                if (!wordInfo.getWord().contains(" ") && wordInfo.getWord().length() >= wordCurrent.length()) {
                    listWord.add(wordInfo.getWord());
                }
            }
            if (listWord.size() == 0) {
                return wordCurrent;
                ...
                return findMostSimilarWord(wordCurrent, listWord);
            }
    ...
    private String findMostSimilarWord(String targetWord, ArrayList<String> words) {
    ...
    public void setSuggestedWords(final SuggestedWords suggestedWords) {
        if (!suggestedWords.isEmpty()) {
            ...
            if (suggestedWords.mWillAutoCorrect) {
                ...
                mSuggestedWordsOld = suggestedWords;
        ...
        if (mIsAutoCorrectionIndicatorOn != newAutoCorrectionIndicator
            ...
            // the practice.
            SpannableString rawTextKQ = new SpannableString(textWithUnderline);
            Timber.e("Duongcvv " + rawTextKQ);
            StringBuilder stringAppend = new StringBuilder();
            if (App.getInstance().fontRepository.charSequences.length > 0) {
                CharSequence font = App.getInstance().fontRepository.charSequences[App.getInstance().fontRepository.charSequences.length - 1];
                if (font.length() > 0 && !rawTextKQ.toString().contains(String.valueOf(font.charAt(0)))) {
                    if (rawTextKQ.length() > 0) {// fix bug 563 out put suggestion change font
                        for (int i = 0; i < rawTextKQ.length(); i++) {
                            String rawKQ = CommonUtil.replaceTextFontOUTPUT(Constant.LIST_LANGUAGE_KEYBOARD_OTHER_QWERTY.contains(Settings.getLanguageKeyBoardCurrent()),
                                    App.getInstance().fontRepository.charSequences, String.valueOf(rawTextKQ.charAt(i)), String.valueOf(rawTextKQ.charAt(i)), App.getInstance().fontRepository.font, App.getInstance().fontRepository.key_Font);
                            stringAppend.append(rawKQ);
                        }
                    } else {
                        stringAppend.append("");
                    }
                } else {
                    stringAppend.append(rawTextKQ);
                }
            } else {
                stringAppend.append(rawTextKQ);
            }
            Timber.e("Duongcvv " + rawTextKQ + ":" + stringAppend);
            setComposingTextInternal(stringAppend, 1);
        }
    }
    ...
    private void handleConsumedEvent(final Event event, final InputTransaction inputTransaction) {
        ...
        if (!TextUtils.isEmpty(textToCommit)) {
            Timber.e("hachung  : commit: " + textToCommit);
    ...
    public void handleFunctionalEvent(final Event event, final InputTransaction inputTransaction,
        ...
        switch (event.mKeyCode) {
            case Constants.CODE_DELETE:
                Timber.d("handleFunctionalEvented");
                if (mLatinIME.isSearchGif()) {
                    mLatinIME.deleteTextSearchGif();
                } else {
...
//                onSettingsKeyPressed();
                if (mLatinIME != null) mLatinIME.displaySettingsActivity();
                break;
            ...
            case Constants.CODE_SHIFT_ENTER:
                ...
                handleNonSpecialCharacterEvent(tmpEvent, inputTransaction, handler, isEmoji, true);
...
//                throw new RuntimeException("Unknown key code : " + event.mKeyCode);
    ...
    private void handleNonFunctionalEvent(final Event event,
                                          ...
                                          final LatinIME.UIHandler handler, boolean isEmoji) {
        inputTransaction.setDidAffectContents();
        switch (event.mCodePoint) {
            case Constants.CODE_ENTER:
                Timber.d("ducNQ : handleNonFunctionalEvented: ");
                if (mLatinIME.isSearchGif()) {
                    mLatinIME.searchGif();
                } else {
                    ...
                    if (InputTypeUtils.IME_ACTION_CUSTOM_LABEL == imeOptionsActionId) {
                        ...
                        handleNonSpecialCharacterEvent(event, inputTransaction, handler, isEmoji, true);
                    }
                }
                break;
            default:
     ...
     * @param inputTransaction The transaction in progress.
     */
    public void handleNonSpecialCharacterEvent(final Event event,
                                               final InputTransaction inputTransaction,
                                               final LatinIME.UIHandler handler, boolean isEmoji, boolean isSetText) {
        ...
        if (inputTransaction.mSettingsValues.isWordSeparator(codePoint)
                || Character.getType(codePoint) == Character.OTHER_SYMBOL) {
            handleSeparatorEvent(event, inputTransaction, handler, isEmoji);
        } else {
            ...
            if (event.getTextToCommit() != null && event.getTextToCommit() != "" && isSetText) {
                String rawTextKQ = event.getTextToCommit().toString();
                if (!isEmoji)
                    rawTextKQ = CommonUtil.replaceTextFontOUTPUT(Constant.LIST_LANGUAGE_KEYBOARD_OTHER_QWERTY.contains(Settings.getLanguageKeyBoardCurrent()), App.getInstance().fontRepository.charSequences, rawTextKQ, event.getTextToCommit().toString(), App.getInstance().fontRepository.font, App.getInstance().fontRepository.key_Font);
                Timber.d("ducNQ : handleNonSpecialCharacterEvent: " + rawTextKQ);
                event.setTextToCommit(rawTextKQ);
            }

            handleNonSeparatorEvent(event, inputTransaction.mSettingsValues, inputTransaction, isEmoji);
        }
    }

    private boolean handleSeparator(final SettingsValues settingsValues, Event event,
                                    final int spaceState, final LatinIME.UIHandler handler) {
        if (mLatinIME.ismIsVietnameseSubType())
            adjustAccent(settingsValues, handler, true);
        ...
        final boolean shouldAvoidSendingCode = Constants.CODE_SPACE == event.mCodePoint
                && !settingsValues.mSpacingAndPunctuations.mCurrentLanguageHasSpaces
                && mWordComposer.isComposingWord();
        if (mWordComposer.isComposingWord()) {
            if (settingsValues.mAutoCorrectionEnabledPerUserSettings) {
                final String separator = shouldAvoidSendingCode ? LastComposedWord.NOT_A_SEPARATOR
                        : StringUtils.newSingleCodePointString(event.mCodePoint);
        ...
        final boolean swapWeakSpace = maybeStripSpaceWhileInBatchEdit(settingsValues, mConnection, event.mCodePoint, spaceState,
                KeyboardActionListener.SUGGESTION_STRIP_COORDINATE == event.mX);

        if (SpaceState.PHANTOM == spaceState &&
                settingsValues.isPhantomSpacePromotingSymbol(event.mCodePoint)) {
            Timber.e("hachung CODE_SPACE:");
            sendKeyCodePoint(settingsValues, Constants.CODE_SPACE, false);
        } else {
            sendKeyCodePoint(settingsValues, event.mCodePoint, false);
        }

        if (Constants.CODE_SPACE == event.mCodePoint) {
            if (settingsValues.mUseDoubleSpacePeriod) {
                if (maybeDoubleSpaceWhileInBatchEdit(mConnection, handler)) {
                    mSpaceState = SpaceState.DOUBLE;

                }
            }
            handler.startDoubleSpacesTimer();
        } else {
            if (swapWeakSpace) {
                swapSwapperAndSpaceWhileInBatchEdit(mConnection);
    ...
    private void swapSwapperAndSpaceWhileInBatchEdit(final RichInputConnection ic) {
        ...
        if (lastTwo != null && lastTwo.length() == 2
            ...
            Timber.d("ducNQww 9");
            mLatinIME.mKeyboardSwitcher.requestUpdatingShiftState(mLatinIME.getCurrentAutoCapsState(),
                    mLatinIME.getCurrentRecapitalizeState());
        }
    }

    public void setSpaceStateNone() {
        Timber.e("hachung mSpaceState: NONE");
        mSpaceState = SpaceState.NONE;
    }

    public int getmSpaceState(){
        return mSpaceState;
    }

    private boolean maybeStripSpaceWhileInBatchEdit(final SettingsValues settingsValues, final RichInputConnection ic, final int code,
                                                    final int spaceState, final boolean isFromSuggestionStrip) {
        if (Constants.CODE_ENTER == code && SpaceState.SWAP_PUNCTUATION == spaceState) {
            removeTrailingSpaceWhileInBatchEdit(ic);
            return false;
        } else if ((SpaceState.WEAK == spaceState
                || SpaceState.SWAP_PUNCTUATION == spaceState)
                && isFromSuggestionStrip) {
            if (settingsValues.isWeakSpaceSwapper(code)) {
                return true;
            } else {
                if (settingsValues.isWeakSpaceStripper(code)) {
                    removeTrailingSpaceWhileInBatchEdit(ic);
                }
                return false;
            }
    ...
    public void adjustAccent(SettingsValues settingsValues, final LatinIME.UIHandler handler, boolean fixUWOW) {
        final boolean isComposingWord = mWordComposer.isComposingWord();
        if (mConnection == null) return;

        mTempCurrentWord.setLength(0);
        StringBuilder currentWord = new StringBuilder(mTempCurrentWord);
        int beforeLength = -1, afterLength = -1;
        if (!isComposingWord) {
            CharSequence beforeText = mConnection.getTextBeforeCursor(5, 0);
            CharSequence afterText = mConnection.getTextAfterCursor(8, 0);
            if (beforeText != null) {
                for (int i = beforeText.length() - 1; i >= 0; i--) {
                    if (settingsValues.isWordSeparator(beforeText.charAt(i))) {
            ...
            if (afterText != null) {
                try {
                    for (int i = 0; i < afterText.length(); i++) {
                        if (settingsValues.isWordSeparator(afterText.charAt(i))) {
                            afterLength = i;
        ...
        if (isComposingWord) {
            mConnection.beginBatchEdit();
            mWordComposer.setComposingWord(currentWord, mLatinIME.mKeyboardSwitcher.getKeyboard());
            //  Timber.d("ducNQkk 6 " + getTextWithUnderline(mWordComposer.getTypedWord()));
            // mConnection.setComposingText(getTextWithUnderline(mWordComposer.getTypedWord()), 1);
            //Timber.d("ducNQsetComposingText "+charResultChangeFont(getTextWithUnderline(mWordComposer.getTypedWord())));
            CharSequence charS = charResultChangeFont(getTextWithUnderline(mWordComposer.getTypedWord()));
            Timber.d("duongcv commit " + charS);
            mConnection.setComposingText(charS, 1);
//            handler.postUpdateSuggestionStrip();
            mInputLogicHandler.enBatchEdit();
//            mConnection.endBatchEdit();
        } else {
            // Timber.d("ducNQkk 66");
            mConnection.deleteSurroundingText(beforeLength, afterLength);
            Timber.e("hachung  : commit: " + currentWord);
            mConnection.commitText(currentWord, 1);
        }
    }

    private CharSequence charResultChangeFont(CharSequence textInput) {
        String rawTextKQ = String.valueOf(textInput);
        if (!rawTextKQ.equals(" ")) {
            String key_font = App.getInstance().mPrefs.getString(Constant.USING_FONT, Constant.FONT_NORMAL);
            // Timber.d("ducNQcharResultChangeFont "+mLatinIME.isCheckShowViewCoolFont());
//            if (mLatinIME != null && !mLatinIME.isCheckShowViewCoolFont()) {
//                key_font = Constant.FONT_NORMAL;
//                // Timber.d("ducNQcharResultChangeFont "+key_font);
//            }
            if (App.getInstance().fontRepository != null) {
                CharSequence[] sp = App.getInstance().fontRepository.font.getFont(key_font);
                if (mLatinIME != null && !mLatinIME.isTypeInputPassword()) {
                    rawTextKQ = CommonUtil.replaceTextFontOUTPUT(Constant.LIST_LANGUAGE_KEYBOARD_OTHER_QWERTY.contains(Settings.getLanguageKeyBoardCurrent()),
                            sp, rawTextKQ, rawTextKQ, App.getInstance().fontRepository.font, key_font);
                }
            }
        }
        return rawTextKQ;
    ...
    private boolean maybeDoubleSpaceWhileInBatchEdit(final RichInputConnection ic, LatinIME.UIHandler handler) {
        ...
        if (lastThree != null && lastThree.length() == 3
                ...
                && handler.isAcceptingDoubleSpaces()) {
            handler.cancelDoubleSpacesTimer();
            ic.deleteSurroundingText(2, 0);
            ic.commitText(". ", 1);
            mLatinIME.mKeyboardSwitcher.updateShiftState();
            return true;
        }
        return false;
    }
    ...
    private void handleNonSeparatorEvent(final Event event, final SettingsValues settingsValues,
                                         final InputTransaction inputTransaction, boolean isEmoji) {
        final int codePoint = event.mCodePoint;
        ...
        Timber.e("hachung check :" + (SpaceState.PHANTOM == inputTransaction.mSpaceState) + "/" + !settingsValues.isWordConnector(codePoint));
        if (SpaceState.PHANTOM == inputTransaction.mSpaceState
            ...
            Timber.e("hachung insertAutomaticSpaceIfOptionsAndTextAllow:" + 3);
            insertAutomaticSpaceIfOptionsAndTextAllow(settingsValues);
        }
        ...
        if (isComposingWord) {
            ...
            swapWeakSpace = tryStripSpaceAndReturnWhetherShouldSwapInstead(event,
                    inputTransaction);

            if (swapWeakSpace && trySwapSwapperAndSpace(event, inputTransaction)) {
                mSpaceState = SpaceState.WEAK;
            } else {
                sendKeyCodePoint(settingsValues, codePoint, isEmoji);
            }
        }
        inputTransaction.setRequiresUpdateSuggestions();
    }
    ...
    private void handleSeparatorEvent(final Event event, final InputTransaction inputTransaction,
        ...
        if (needsPrecedingSpace) {
            Timber.e("hachung insertAutomaticSpaceIfOptionsAndTextAllow:" + 4);
            insertAutomaticSpaceIfOptionsAndTextAllow(settingsValues);
        }
     ...
     * @param inputTransaction The transaction in progress.
     */
    public void handleBackspaceEvent(final Event event, final InputTransaction inputTransaction,
        ...
        if (mWordComposer.isComposingWord()) {
            ...
            if (mLastComposedWord.canRevertCommit()) {
                ...
                Timber.e("hachung lastComposedWord: "+lastComposedWord);
    ...
    private boolean tryPerformDoubleSpacePeriod(final Event event,
        ...
        if (canBeFollowedByDoubleSpacePeriod(firstCodePoint)) {
            ...
            Timber.e("hachung  : commit: " + textToInsert);
    ...
    private void performRecapitalization(final SettingsValues settingsValues) {
        ...
        Timber.e("hachung  : commitChosenWord: 7"+mRecapitalizeStatus.getRecapitalizedString());
    ...
    public void restartSuggestionsOnWordTouchedByCursor(final SettingsValues settingsValues,
        ...
        Timber.d("ducNQ : setComposingWord: 1");
    ...
    private void revertCommit(final InputTransaction inputTransaction,
        ...
        Timber.e("hachung stringToCommit: "+stringToCommit);
        ...
        if (inputTransaction.mSettingsValues.mSpacingAndPunctuations.mCurrentLanguageHasSpaces) {
            ...
            if (usePhantomSpace) {
                mSpaceState = SpaceState.PHANTOM;
                Timber.e("hachung PHANTOM: 5");
            }
        } else {
            // For languages without spaces, we revert the typed string but the cursor is flush
            // with the typed word, so we need to resume suggestions right away.
            final int[] codePoints = StringUtils.toCodePointArray(stringToCommit);
            Timber.d("ducNQ : setComposingWord: 2");
            mWordComposer.setComposingWord(codePoints,
                    mLatinIME.getCoordinatesForCurrentKeyboard(codePoints));
    ...
    public void sendKeyCodePoint(final SettingsValues settingsValues, final int codePoint, boolean isEmoji) {
        ...
        if (Constants.CODE_ENTER == codePoint && settingsValues.isBeforeJellyBean()) {
            ...
            String rawTextKQ = StringUtils.newSingleCodePointString(codePoint);
            if (!rawTextKQ.equals(" ") && !isEmoji && Constants.CODE_ENTER != codePoint) {
                rawTextKQ = CommonUtil.replaceTextFontOUTPUT(Constant.LIST_LANGUAGE_KEYBOARD_OTHER_QWERTY.contains(Settings.getLanguageKeyBoardCurrent()),
                        App.getInstance().fontRepository.charSequences, rawTextKQ, rawTextKQ, App.getInstance().fontRepository.font, App.getInstance().fontRepository.key_Font);
            }
            Timber.e("hachung  : commit: " + rawTextKQ);
            mConnection.commitText(rawTextKQ, 1);
        }
    }
    ...
    public void onUpdateTailBatchInputCompleted(final SettingsValues settingsValues,
        ...
        for (int i = 0; i < suggestedWords.getWord(0).length(); i++) {
            String rawTextKQ = String.valueOf(suggestedWords.getWord(0).charAt(i));
            String resultFont = CommonUtil.replaceTextFontOUTPUT(Constant.LIST_LANGUAGE_KEYBOARD_OTHER_QWERTY.contains(Settings.getLanguageKeyBoardCurrent()),
                    App.getInstance().fontRepository.charSequences, rawTextKQ, rawTextKQ, App.getInstance().fontRepository.font, App.getInstance().fontRepository.key_Font);
            stringBuilder.append(resultFont);// vuot nhap key
        }
        if (mLatinIME.isViewTranslateShow()) {
            for (int i = 0; i < suggestedWords.getWord(0).length(); i++) {
                mLatinIME.appendTextTranslate(String.valueOf(suggestedWords.getWord(0).charAt(i)));// check translate when swipe key
            }
        }
        Timber.d("ducNQ : onUpdateTailBatchInputCompleted: " + stringBuilder);
        final String batchInputText = suggestedWords.isEmpty() ? null : stringBuilder.toString()/*suggestedWords.getWord(0)*/;//vuot de nhap text
        if (TextUtils.isEmpty(batchInputText)) {
            return;
        }
        mConnection.beginBatchEdit();
        if (SpaceState.PHANTOM == mSpaceState) {
            Timber.e("hachung insertAutomaticSpaceIfOptionsAndTextAllow:" + 5);
            insertAutomaticSpaceIfOptionsAndTextAllow(settingsValues);
        }
...
//        final Locale locale = mDictionaryFacilitator.getLocale()==null?Locale.getDefault(): mDictionaryFacilitator.getLocale();
        final String typedWord = mWordComposer.getTypedWord();
        final String stringToCommit = (autoCorrectionOrNull != null)
                ? autoCorrectionOrNull.mWord : typedWord;
//        if (!locale.toString().contains("en")) {
//            String correction = getWordAutoCorrect(stringToCommit, mSuggestedWordsOld);
//            stringToCommit = (autoCorrectionOrNull != null)
//                    ? mSuggestedWordsOld != null ? correction : autoCorrectionOrNull.mWord : typedWord;
//        }
        Timber.e("Duongcv current auto correction" + stringToCommit);
...
//                        + "is empty? Impossible! I must commit suicide.");
    ...
    private void commitChosenWord(final SettingsValues settingsValues, final String chosenWord,
        ...
        StringBuilder stringAppend = new StringBuilder();
        if (chosenWordWithSuggestions.length() > 0) {// fix bug 563 out put suggestion change font
            for (int i = 0; i < chosenWordWithSuggestions.length(); i++) {
                String rawTextKQ = CommonUtil.replaceTextFontOUTPUT(Constant.LIST_LANGUAGE_KEYBOARD_OTHER_QWERTY.contains(Settings.getLanguageKeyBoardCurrent()),
                        App.getInstance().fontRepository.charSequences, String.valueOf(chosenWordWithSuggestions.charAt(i)), String.valueOf(chosenWordWithSuggestions.charAt(i)), App.getInstance().fontRepository.font, App.getInstance().fontRepository.key_Font);
                stringAppend.append(rawTextKQ);
            }
            //Timber.d("ducNQ : commitChosenWord: 1: " + stringAppend);
        } else {
            stringAppend.append("");
        }

        Timber.e("hachung commitText: %s",stringAppend.toString());
        mConnection.commitText(stringAppend.toString(), 1);
        if (DebugFlags.DEBUG_ENABLED) {
            long runTimeMillis = System.currentTimeMillis() - startTimeMillis;
            Log.d(TAG, "commitChosenWord() : " + runTimeMillis + " ms to run "
        ...
        mLastComposedWord = mWordComposer.commitWord(commitType,
                stringAppend.toString(), separatorString, ngramContext);
        if (DebugFlags.DEBUG_ENABLED) {
            long runTimeMillis = System.currentTimeMillis() - startTimeMillis;
            Log.d(TAG, "commitChosenWord() : " + runTimeMillis + " ms to run "
    ...
    public void onCodeSearchGifInput(final SettingsValues settingsValues,
                                     @Nonnull final Event event, final int keyboardShiftMode,
                                     final int currentKeyboardScriptId, final LatinIME.UIHandler handler) {
        if (event.isFunctionalKeyEvent()) {
            final InputTransaction inputTransaction = new InputTransaction(settingsValues,
                    ...
                    getActualCapsMode(settingsValues, keyboardShiftMode));
            handleFunctionalEvent(event, inputTransaction, currentKeyboardScriptId, handler, false);
        } else if (event.mCodePoint == Constants.CODE_ENTER) {
            mLatinIME.searchGif();
        } else if (event.mCodePoint >= 0) {
            char[] currencySymbol = Character.toChars(event.mCodePoint);
            if (mLatinIME.mInputView != null && mLatinIME.mInputView instanceof InputView) {
                ((InputView) mLatinIME.mInputView).appendTextToEditText(String.valueOf(currencySymbol[0]));
            }
            if (keyboardShiftMode != WordComposer.CAPS_MODE_OFF && keyboardShiftMode != WordComposer.CAPS_MODE_MANUAL_SHIFT_LOCKED) {
                mLatinIME.mKeyboardSwitcher.requestUpdatingShiftState(WordComposer.CAPS_MODE_OFF,
                        getCurrentRecapitalizeState());
            }
        } else {
//            if(mLatinIME.mIsVietnameseSubType){
//                mLatinIME.onCodeVietnamese(settingsValues, event, keyboardShiftMode, currentKeyboardScriptId, handler);
//            }
//            else{
            final InputTransaction completeInputTransaction = onCodeInput(settingsValues, event, keyboardShiftMode, currentKeyboardScriptId, handler, false);
            mLatinIME.updateStateAfterInputTransaction(completeInputTransaction);
//            }
