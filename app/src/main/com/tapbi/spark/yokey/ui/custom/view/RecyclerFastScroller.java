...
import com.tapbi.spark.yokey.util.RecyclerFastScrollerUtils;
...
public class RecyclerFastScroller extends FrameLayout {
    ...
    public RecyclerFastScroller(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        mTouchTargetWidth = a.getDimensionPixelSize(
                ...
                RecyclerFastScrollerUtils.convertDpToPx(context, 30));//create width when touch do not width view
        ...
        int fortyEightDp = RecyclerFastScrollerUtils.convertDpToPx(context, 48);//height scroll bar
        ...
        mHiddenTranslationX = (RecyclerFastScrollerUtils.isRTL(getContext()) ? -1 : 1) * eightDp * 2;
        mHide = new Runnable() {
            @Override
            public void run() {
...
//        });
    ...
    public void setTouchTargetWidth(int touchTargetWidth) {
        ...
        int eightDp = RecyclerFastScrollerUtils.convertDpToPx(getContext(), 8);
        mBarInset = mTouchTargetWidth - eightDp;//set width scroller
    ...
    private void updateHandleColorsAndInset() {
        ...
        @SuppressLint("UseCompatLoadingForDrawables") Drawable myIcon = getResources().getDrawable(R.drawable.draw_press_scroller);
        if (!RecyclerFastScrollerUtils.isRTL(getContext())) {
            drawable.addState(View.PRESSED_ENABLED_STATE_SET,
                    new InsetDrawable(myIcon, mBarInset, 0, RecyclerFastScrollerUtils.convertDpToPx(getContext(), 5f), 0));
            drawable.addState(View.EMPTY_STATE_SET,
                    new InsetDrawable(myIcon, mBarInset, 0, RecyclerFastScrollerUtils.convertDpToPx(getContext(), 5f), 0));
        } else {
            drawable.addState(View.PRESSED_ENABLED_STATE_SET,
                    new InsetDrawable(myIcon, 0, 0, mBarInset, 0));
            drawable.addState(View.EMPTY_STATE_SET,
                    new InsetDrawable(myIcon, 0, 0, mBarInset, 0));
        }
    ...
    private void updateBarColorAndInset() {
        Drawable drawable;
        if (!RecyclerFastScrollerUtils.isRTL(getContext())) {
            drawable = new InsetDrawable(new ColorDrawable(mBarColor), mBarInset, 0, RecyclerFastScrollerUtils.convertDpToPx(getContext(), 5f), 0);
        } else {
